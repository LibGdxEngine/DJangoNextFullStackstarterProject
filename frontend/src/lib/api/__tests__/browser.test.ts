...
it("deduplicates concurrent protected 401s for the same session", async () => {
  ...
  mockSession.mockResolvedValue({ sessionGeneration: "old", expires: "2099-01-01" });
  ...
  await vi.waitFor(() => expect(expire).toHaveBeenCalledExactlyOnceWith("old"));
...
it("ignores a delayed failure from an older session", async () => {
  const { apiClient, registerSessionExpiry } = await import("../browser");
  mockSession.mockResolvedValueOnce({ sessionGeneration: "old", expires: "2099-01-01" });
  mockSession.mockResolvedValue({ sessionGeneration: "new", expires: "2099-01-01" });
